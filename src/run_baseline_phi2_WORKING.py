import csv
import re
import subprocess
import time
from pathlib import Path
from datetime import datetime

# ====== CONFIG ======
BIN   = str(Path.home() / "llama.cpp/build/bin/llama-completion")
MODEL = str(Path.home() / "edge-ai/models/phi-2-Q4_K_M.gguf")

PROMPTS_CSV = "baseline_prompts.csv"

TIMEOUT_S = 60
THREADS = 4
CTX = 512

# keep outputs short (Phi-2 can ramble)
N_PRED_NUM = 32   # AR/ALG/WP
N_PRED_LOG = 8    # LOG

GRAMMAR_YESNO = "grammar_yesno.gbnf"
GRAMMAR_INT   = "grammar_phi2_answer_int.gbnf"

RUN_TAG = datetime.now().strftime("%Y%m%d_%H%M%S")
RUN_DIR = Path("runs") / RUN_TAG
LOG_DIR = RUN_DIR / "logs"
RUN_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR.mkdir(parents=True, exist_ok=True)

YESNO_ANY = re.compile(r"\b(Yes|No)\b")
INT_ANY   = re.compile(r"[+-]?\d+")

def kind_from_category(category: str) -> str | None:
    cat = category.strip().upper()
    if cat == "LOG":
        return "log"
    if cat in ("AR", "ALG", "WP"):
        return "num"
    return None

def grammar_for(variant: str, kind: str) -> str | None:
    # Two baseline variants:
    # - phi2_nogrammar: no grammar
    # - phi2_grammar:   yes/no grammar for LOG, int-answer grammar for NUM
    if variant == "phi2_nogrammar":
        return None
    if kind == "log":
        return GRAMMAR_YESNO
    if kind == "num":
        return GRAMMAR_INT
    return None

def n_predict_for(kind: str) -> int:
    return N_PRED_LOG if kind == "log" else N_PRED_NUM

def extract_num(stdout: str) -> str:
    # Best rule: first line containing Answer:
    for line in stdout.splitlines():
        if "Answer:" in line:
            m = INT_ANY.search(line)
            if m:
                return m.group(0)
    # fallback: last integer anywhere in stdout
    hits = INT_ANY.findall(stdout)
    return hits[-1] if hits else ""

def extract_yesno(stdout: str) -> str:
    hits = YESNO_ANY.findall(stdout)
    return hits[-1] if hits else ""

def compute_correct(expected: str, got: str, kind: str) -> int:
    if not expected or not got:
        return 0
    exp = expected.strip()
    got = got.strip()
    if kind == "log":
        return 1 if exp.lower() == got.lower() else 0
    try:
        return 1 if int(exp) == int(got) else 0
    except Exception:
        return 0

def error_code(status: str, got: str, correct: int) -> str:
    if status == "timeout":
        return "E4"
    if not got.strip():
        return "E2"
    return "E0" if correct == 1 else "E1"

def run_llm(prompt: str, grammar_file: str | None, n_predict: int):
    cmd = [
        BIN, "-m", MODEL,
        "-t", str(THREADS),
        "-c", str(CTX),
        "-n", str(n_predict),
        "-no-cnv",
        "--temp", "0",
        "-p", prompt,
    ]
    if grammar_file:
        cmd += ["--grammar-file", grammar_file]

    t0 = time.time()
    try:
        p = subprocess.run(
            cmd,
            capture_output=True,  # keeps stdout and stderr separate
            text=True,
            timeout=TIMEOUT_S
        )
        lat = time.time() - t0
        return "ok", lat, p.stdout, p.stderr
    except subprocess.TimeoutExpired as e:
        lat = time.time() - t0
        out = e.stdout if isinstance(e.stdout, str) else ""
        err = e.stderr if isinstance(e.stderr, str) else ""
        return "timeout", lat, out, err

def main():
    with open(PROMPTS_CSV, newline="") as f:
        prompts = list(csv.DictReader(f))

    variants = ["phi2_nogrammar", "phi2_grammar"]
    out_rows = []

    for variant in variants:
        for r in prompts:
            rid = r.get("id", "").strip()
            cat = r.get("category", "").strip()
            prompt = r.get("prompt", "")
            expected = r.get("expected_answer", "").strip()

            kind = kind_from_category(cat)
            if not kind:
                continue

            grammar = grammar_for(variant, kind)
            n_pred = n_predict_for(kind)

            status, lat, stdout, stderr = run_llm(prompt, grammar, n_pred)

            # IMPORTANT: parse ONLY stdout (stderr contains perf numbers)
            if status == "ok":
                got = extract_yesno(stdout) if kind == "log" else extract_num(stdout)
            else:
                got = ""

            corr = compute_correct(expected, got, kind)
            ecode = error_code(status, got, corr)

            # Save raw logs for auditing
            log_path = LOG_DIR / f"{rid}__{variant}.txt"
            log_path.write_text(
                "=== PROMPT ===\n" + prompt +
                "\n\n=== STDOUT ===\n" + (stdout or "") +
                "\n\n=== STDERR ===\n" + (stderr or ""),
                encoding="utf-8",
                errors="replace"
            )

            print(f"{rid} [{variant}] {status} {lat:.2f}s -> {got}")

            out_rows.append({
                "id": rid,
                "category": cat,
                "variant": variant,
                "grammar_used": grammar or "",
                "prompt": prompt,
                "expected_answer": expected,
                "final_output": got,
                "correct": str(corr),
                "error_code": ecode,
                "latency_s": f"{lat:.4f}",
            })

    out_csv = RUN_DIR / "baseline_results.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(out_rows[0].keys()))
        w.writeheader()
        w.writerows(out_rows)

    print(f"\n✅ wrote {out_csv}")
    print(f"📁 raw logs in {LOG_DIR}")

if __name__ == "__main__":
    main()
