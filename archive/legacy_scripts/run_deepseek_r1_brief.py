...
def main():
    ...
    ap.add_argument("--n_pred", type=int, default=20, help="Max tokens for brief answers")
    ...
    for i, row in enumerate(rows, start=1):
        ...
        # Build prompt with Answer: suffix for DeepSeek-R1 brief mode
        ...
        for _ in range(args.warmup_per_prompt):
            try:
                requests.post(
                    args.server_url,
                    json={"prompt": prompt, "n_predict": 8, "temperature": 0.0, "stop": ["\n"]},
        ...
        for t in range(1, args.repeats + 1):
            ...
            timed_out = False
            try:
                # DeepSeek-R1 brief mode: use stop sequence to get immediate answer
                r = requests.post(
                    args.server_url,
                    json={
                        "prompt": prompt,
                        "n_predict": int(args.n_pred),
                        "temperature": 0.0,
