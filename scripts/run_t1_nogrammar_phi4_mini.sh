...
# Tier-1 Phi-4-Mini Q6_K baseline without grammar constraints + energy logging
...
MODEL_FILE="${MODEL_FILE:-/home/stopthecap10/edge-ai/models/microsoft_Phi-4-mini-instruct-Q6_K.gguf}"
REPEATS="${REPEATS:-3}"
TIMEOUT_S="${TIMEOUT_S:-20}"
WARMUP_PER_PROMPT="${WARMUP_PER_PROMPT:-1}"
N_PRED_NUM="${N_PRED_NUM:-12}"
N_PRED_LOG="${N_PRED_LOG:-6}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"

OUT_CSV="outputs/T1_phi4_mini_q6k_nogrammar_r${REPEATS}_${RUN_ID}.csv"
TRIALS_CSV="outputs/T1_phi4_mini_q6k_nogrammar_r${REPEATS}_${RUN_ID}_trials.csv"

python3 src/log_run_energy.py \
  --system phi4_mini_q6k_nogrammar \
  --tier T1 \
  --csv_used "$CSV_PATH" \
