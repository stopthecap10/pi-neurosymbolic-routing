...
# Tier-1 Qwen2.5-Math-7B Q4 baseline without grammar constraints + energy logging
...
OUT_CSV="outputs/T1_qwen2.5_math_7b_q4_nogrammar_r${REPEATS}_${RUN_ID}.csv"
TRIALS_CSV="outputs/T1_qwen2.5_math_7b_q4_nogrammar_r${REPEATS}_${RUN_ID}_trials.csv"

python3 src/log_run_energy.py \
  --system qwen2.5_math_7b_q4_nogrammar \
  --tier T1 \
  --csv_used "$CSV_PATH" \
