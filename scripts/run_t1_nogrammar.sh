...
# Tier-1 Phi-2 Q8 baseline without grammar constraints + energy logging
...
MODEL_FILE="${MODEL_FILE:-/home/stopthecap10/edge-ai/models/phi-2.Q8_0.gguf}"
REPEATS="${REPEATS:-3}"
...
OUT_CSV="outputs/T1_phi2_q8_nogrammar_r${REPEATS}_${RUN_ID}.csv"
TRIALS_CSV="outputs/T1_phi2_q8_nogrammar_r${REPEATS}_${RUN_ID}_trials.csv"

python3 src/log_run_energy.py \
  --system phi2_q8_nogrammar \
  --tier T1 \
  --csv_used "$CSV_PATH" \
